...
\end{equation}
where $\hat{\mathbf{w}} = [\hat{\mathbf{w}}_y \quad \hat{\mathbf{w}}_x]^\intercal$, and
\begin{equation}
\mathbf{X} = 
	\left[ 
	\begin{array}{ccccccc}
		y_{(p)} & y_{(p-1)} & \cdots & y_{(0)} & x_{1(p)} & \cdots &  x_{m(p)} \\
		y_{(p+1)} & y_{(p)} & \cdots & y_{(1)} & x_{1(p+1)} & \cdots &  x_{m(p+1)} \\	
		\vdots  & \vdots  &          & \vdots  & \vdots   &        &  \vdots   \\
		y_{(n-p-1)} & y_{(n-p)} & \cdots & y_{(n-2p-1)} & x_{1(n-p-1)} & \cdots &  x_{m(n-p-1)} \\			
	\end{array}
\right].
\end{equation}
Thus, we will discard the first $p$ values from each time series $y, x_i$ such that  $\mathbf{y} = [y_{(p+1)} \quad y_{(p+2)} \quad \cdots \quad y_{(n-p)}]^\intercal$. When we solve Equation (\ref{eq:ols}), we should probably use singular value decomposition of $\mathbf{X}$ and calculate its pseudoinverse to avoid any problems with singular matrices. See \cite{lay2016linear}.

After having found the optimal weights $\hat{\mathbf{w}}$, we will probably want to evaluate the model for some input time series $x_i$ and a given starting condition $y_0$. We will assume steady state initially, such that the predicted sequence $\mathbf{y}_{pred}$ becomes
\begin{equation}
\begin{aligned}
	&\mathbf{y}_{pred} = 
	\left[ 
...
\right]
	...
	&= \left[ 
		\begin{array}{ccccccc}
			y_{(0)} & y_{(0)} & \cdots & y_{(0)} & x_{1(0)} & \cdots &  x_{m(0)} \\
			y_{(1)} & y_{(0)} & \cdots & y_{(0)} & x_{1(1)} & \cdots &  x_{m(1)} \\	
			y_{(2)} & y_{(1)} & \cdots & y_{(0)} & x_{1(2)} & \cdots &  x_{m(2)} \\
			\vdots  & \vdots  &        & \vdots  & \vdots   &        &  \vdots   \\
			y_{(p+1)} & y_{(p)} & \cdots & y_{(1)}  \\
			\vdots  & \vdots  &        & \vdots     \\
			y_{(n-1)} & y_{(n-1)} & \cdots & y_{(n-p)} & x_{1(n-1)} & \cdots &  x_{m(n-1)} \\			
		\end{array}
	\right]
	\left[ 
\begin{array}{c}
...
\end{aligned}
\end{equation}
The model will thus have a run-up period of $p$ time steps, and we see that since the terms $y_{(t)}$ appear on both sides of the equation, $\mathbf{y}_{pred}$ must be calculated sequentially.
